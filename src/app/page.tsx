...
export default function PicktoApp() {
  ...
  const handleDocumentSubmit = async () => {
    ...
    try {
      ...
      // Pass all the data to the Cloud Function
      const result = await uploadDocumentsDriver({
        riderId,
        files: preparedFiles,
        fullName,
        city,
      ...
      if (!(result.data as any).success) {
        throw new Error(
          (result.data as any).message || 'Backend submission failed.'
        );
      }
      ...
      if (docSnap.exists()) {
        ...
        if (riderData.status === 'verified') {
          toast({
            ...
            description: 'Welcome to Pickto!',
          });
          setAppState('DASHBOARD');
        } else {
          toast({
            title: 'Status Pending',
            description: `Your application is still ${riderData.status || 'under review'}.`,
          });
        }
      } else {
        toast({
          variant: 'destructive',
          title: 'Error',
          description: 'Could not find your application details.',
        });
      }
    } catch (error) {
  ...
  const renderOnboarding = () => {
    ...
    const file = onboardingData[currentStepConfig.fileField as keyof typeof onboardingData];
                ...
                <div className='grid grid-cols-2 gap-4'>
                  <div
                    className={`border rounded-lg p-4 flex flex-col items-center gap-2 cursor-pointer transition-all ${
                      onboardingData.vehicleType === '2 Wheeler' ? 'border-primary bg-primary/5' : 'opacity-50'
                    }`}
                  ...
                  <div
                    className={`border rounded-lg p-4 flex flex-col items-center gap-2 cursor-pointer transition-all ${
                      onboardingData.vehicleType === 'Bicycle' ? 'border-primary bg-primary/5' : 'opacity-50'
                    }`}
            ...
            <div className='space-y-6 animate-in fade-in'>
              ...
              <div className='space-y-2'>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setOnboardingData((prev) => ({ ...prev, [currentStepConfig.field as string]: e.target.value }))
                  }
                />
              </div>
            </div>
    ...
    <div className='screen-content flex flex-col items-center justify-center h-full text-center space-y-6 bg-white'>
      ...
      <div className='space-y-2'>
        ...
        <p className='text-muted-foreground px-8'>
          We are currently verifying your documents. This usually takes 24-48 hours. We'll notify you once you're
          approved.
        </p>
      </div>
      <Button type='button' variant='outline' className='w-full max-w-xs' onClick={handleRefreshStatus}>
        Refresh Status
      </Button>
    </div>
  );
      ...
      <div className='space-y-4'>
        <Card
          className={`transition-all duration-300 ${
            isOnline ? 'border-success bg-success/5 shadow-lg shadow-success/10' : 'bg-white'
          }`}>
        ...
        <Card className='bg-white'>
          <CardHeader className='p-4 pb-0 flex flex-row items-center justify-between'>
            ...
            <Badge variant='outline' className='text-[10px] uppercase font-bold text-warning border-warning'>
              ₹ 200 Reward
            </Badge>
          </CardHeader>
    ...
    <div className='screen-content h-full bg-[#ECF1F6] space-y-6'>
      ...
      <Card className='bg-primary text-white border-none shadow-xl overflow-hidden relative'>
        ...
        <CardContent className='p-8 space-y-4'>
          ...
          <Button type='button' className='w-full bg-white text-primary hover:bg-white/90 font-bold py-6'>
            Withdraw Now
          </Button>
        </CardContent>
      </Card>
      ...
      <Tabs defaultValue='daily' className='w-full'>
        <TabsList className='w-full grid grid-cols-2 bg-white p-1 rounded-xl h-12 shadow-sm'>
          <TabsTrigger value='daily' className='rounded-lg font-bold data-[state=active]:bg-primary data-[state=active]:text-white'>
            Daily
          </TabsTrigger>
          <TabsTrigger value='weekly' className='rounded-lg font-bold data-[state=active]:bg-primary data-[state=active]:text-white'>
            Weekly
          </TabsTrigger>
        </TabsList>
  ...
  const renderProfile = () => (
    <div className='screen-content h-full bg-[#ECF1F6] space-y-6'>
      ...
      <Button type='button' variant='outline' className='w-full text-destructive border-destructive/20 bg-destructive/5 hover:bg-destructive/10'>
        Delete Account
      </Button>
    </div>
  );
          ...
          {[
            ...
            <button
              ...
              className={`flex flex-col items-center gap-1 transition-all ${
                appState === item.id ? 'text-primary scale-110' : 'text-muted-foreground'
              }`}
